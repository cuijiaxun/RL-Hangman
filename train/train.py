...
from env import HangmanEnvFactory
...
def main(cfg):
    ...
    env_fac = HangmanEnvFactory(cfg.env_config)
    env = env_fac(0)
    cfg.model_config["output_dim"] = env.action_space.n

    train_model = PPOTransformerModel(**cfg.model_config).to(cfg.train_device)
    ...
    for epoch in range(cfg.num_epochs):
        ...
        if (epoch+1) % 10 == 0:
            torch.save(train_model.state_dict(), f"ppo_agent-{epoch+1}.pth")
    loops.terminate()
    servers.terminate()
